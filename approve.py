...
def approve(array_index, input_csv):
    input_data = [] 
    ...
    for i in range(1, len(input_data)):
        ...
        nasa_value = row[5]
        source = row[6]
        #finds the new value to be added
        if eu_value == "":
            other_value = nasa_value
        else:
            other_value = eu_value
        #checks if index is in the array
        if str(i - 1) in array_index:
            #update xml
            update_oec_xml(name, category, category_convert(attribute), other_value, source)
            update_oec_xml(name, category, "lastupdate", datetime.datetime.today().strftime('%y/%m/%d'), source)
        else:
            #add row to csv
